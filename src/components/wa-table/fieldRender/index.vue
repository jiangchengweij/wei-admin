<template>
  <!-- Icon -->
  <wa-icon class="ba-icon-dark" v-if="field.render == 'icon'" :name="fieldValue ? fieldValue : field.default ?? ''" />

  <!-- switch -->
  <el-switch
    v-if="field.render == 'switch'"
    @change="onChangeField"
    :model-value="fieldValue ? fieldValue.toString() : field.default ?? '0'"
    :loading="row.loading"
    active-value="1"
    inactive-value="0"
  />
  ...
  <div v-if="field.render == 'image' && fieldValue" class="ba-render-image">
    <el-image :hide-on-click-modal="true" :preview-teleported="true" :preview-src-list="[fullUrl(fieldValue)]" :src="fullUrl(fieldValue)"></el-image>
  </div>
  ...
  <div v-if="field.render == 'url' && fieldValue">
    <el-input :model-value="fieldValue" :placeholder="t('Link address')">
      <template #append>
        <el-button @click="typeof field.click == 'function' ? field.click(row, field, fieldValue, column, index) : openUrl(fieldValue, field)">
          <wa-icon :color="'#606266'" name="el-icon-Position" />
        </el-button>
      </template>
    </el-input>
  </div>
  ...
  <div v-if="field.render == 'customTemplate'" v-html="field.customTemplate ? field.customTemplate(row, field, fieldValue, column, index) : ''"></div>
