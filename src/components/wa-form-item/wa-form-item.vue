...
import { inputTypes } from '/@/components/wa-input'
import type { modelValueTypes, InputAttr, InputData } from '/@/components/wa-input'
import type { FormItemAttr } from './index'
import WaInput from '@/components/wa-input/wa-input.vue'
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    // el-form-item 的默认插槽,生成一个waInput
    const defaultSlot = () => {
      let inputNode = createVNode(WaInput, {
        type: props.type,
    ...
    if (noNeedLabelSlot.includes(props.type)) {
      ...
      return () =>
        createVNode(
          resolveComponent('el-form-item'),
          {
            class: 'wa-input-item-' + props.type,
            prop: props.prop,
            ...props.attr,
            label: props.label,
          },
...
.wa-form-item-label {
  ...
  .wa-form-item-label-tip {
    padding-left: 6px;
    font-size: 12px;
    color: var(--el-text-color-secondary);
  }
}
.wa-form-item-not-support {
  line-height: 15px;
}
.wa-input-item-array :deep(.el-form-item__content) {
  display: block;
  padding-bottom: 32px;
}
</style>
