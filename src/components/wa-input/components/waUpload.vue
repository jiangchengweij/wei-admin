    ...
    >
      ...
      <template v-else #default>
        <template v-if="type == 'image' || type == 'images'">
          ...
          <wa-icon class="ba-upload-icon" name="el-icon-Plus" size="30" color="#c0c4cc" />
        </template>
        <template v-else>
          <el-button v-blur type="primary">
            <wa-icon name="el-icon-Plus" color="#ffffff" />
            <span>{{ $t('Upload') }}</span>
          </el-button>
          <el-button v-blur v-if="!hideSelectFile" @click.stop="state.selectFile.show = true" type="success">
            <wa-icon name="fa fa-th-list" size="14px" color="#ffffff" />
...
import { stringToArray } from '@/components/wa-input/helper'
import { getFileNameFromPath, getArrayKey } from '/@/utils/common'
