...
const topStyleHeight = pagesConfig.topWindow.style.height;
const leftStyleWidth = pagesConfig.leftWindow.style.width;
...
export const useConfig = defineStore(
  ...
  () => {
    ...
    try {
      layout.menuWidth = parseInt(leftStyleWidth.replace(/[^0-9]/ig, ''))
    } catch(e) {
      console.warn('是否正确配置leftWidow.style.width?')
    }

    try {
      layout.headerHeight = parseInt(topStyleHeight.replace(/[^0-9]/ig, ''))
    } catch(e) {
      console.warn('是否正确配置topWindow.style.height?')
    }
    ...
    return { lang, setLang, getColorVal, layout, setLayoutMode, setLayout, onSetLayoutColor, menuWidth }
  }, {
    persist: {
      key: STORE_CONFIG
    }
